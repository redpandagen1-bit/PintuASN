      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-[#1B2B5E]/8 flex items-center justify-center flex-shrink-0">
            <CalendarDays className="w-4.5 h-4.5 text-[#1B2B5E]" />
          </div>
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-0.5">
              Jadwal latihan &amp; pantau konsistensimu
            </p>
          </div>
        </div>
        ...
        {daysLeft !== null && daysLeft > 0 && (
          <div className="text-right bg-[#1B2B5E] px-3 py-1.5 rounded-xl flex-shrink-0">
            <p className="text-sm font-extrabold text-white leading-none">{daysLeft}</p>
            <p className="text-[9px] text-white/60 mt-0.5 uppercase tracking-wide">hari lagi</p>
          </div>
        )}
        {daysLeft !== null && daysLeft === 0 && (
          <span className="text-[10px] font-bold bg-[#F5A623] text-white px-2.5 py-1.5 rounded-xl">
            Hari Ujian! 🎯
          </span>
        )}
        {daysLeft !== null && daysLeft < 0 && (
          <span className="text-[10px] font-bold bg-red-100 text-red-600 px-2.5 py-1.5 rounded-xl">
            Sudah lewat
          </span>
        )}
      </div>
      ...
      <div className="rounded-2xl bg-white border border-slate-100 overflow-hidden shadow-sm">
        ...
        <div className="bg-[#1B2B5E] px-5 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] text-white/50 font-extrabold uppercase tracking-widest">
                Target Ujian
              </p>
              {selectedDate ? (
                ...
                <p className="text-sm text-white/50 leading-tight mt-0.5 italic">
                  Klik tanggal di bawah untuk mengatur
                </p>
              )}
            </div>
          </div>
          {selectedDate && (
            <button
              ...
              className="text-[10px] text-white/50 hover:text-white transition-colors border border-white/15 hover:border-white/30 px-2.5 py-1 rounded-lg"
            >
              Ubah
            </button>
          )}
        ...
        <div className="px-5 pt-4 pb-3">
          ...
          <div className="grid grid-cols-7 mb-1">
            {CAL_HEADERS.map(h => (
              <div key={h} className="text-center text-[10px] font-extrabold text-slate-400 uppercase py-1 tracking-wider">
                {h}
              </div>
            ))}
          </div>
        ...
        {/* Legend */}
        <div className="px-5 pb-3 pt-1 flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-1.5">
            <div className="w-2.5 h-2.5 rounded-full bg-emerald-400" />
            <span className="text-[10px] text-slate-500">Latihan</span>
          </div>
          <div className="flex items-center gap-1.5">
        ...
        <div className="px-5 py-4 space-y-4">
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isActive
                        ? 'bg-[#1B2B5E]/[0.06] border-[#1B2B5E]/25'
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        {isActive && (
                          <span className="text-[10px] text-slate-400 leading-none">{opt.sublabel}</span>
                        )}
                      </div>
                    </div>
                    <div className={cn(
                  ...
                  {DAY_LABELS.map(day => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          'w-10 h-10 rounded-xl text-[10px] font-extrabold border transition-all flex flex-col items-center justify-center gap-0.5',
                          sel
                            ? 'bg-[#F5A623] text-white border-[#F5A623] shadow-sm'
                            : 'bg-white text-slate-500 border-slate-200 hover:border-[#F5A623]/50',
                        )}
                      >
                        <span>{day.short}</span>
            ...
            <div className="flex items-center justify-between bg-slate-50 border border-slate-100 rounded-xl px-3.5 py-2.5">
              ...
              <button
                ...
                className={cn(
                  ...
                  'focus-visible:ring-2 focus-visible:ring-[#1B2B5E] focus-visible:ring-offset-1',
          ...
          {onSavePreference && (
            <button
              ...
              className={cn(
                'w-full py-2.5 rounded-xl text-sm font-extrabold transition-all duration-200',
                saved
                  ? 'bg-emerald-100 text-emerald-700'
                  : selectedDate
                  ? 'bg-[#1B2B5E] text-white hover:bg-[#1B2B5E]/90 shadow-sm active:scale-[0.98]'
