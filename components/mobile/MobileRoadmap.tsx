...
const STEP_CFG: Record<PhaseStatus, {
  ...
  completed: {
    circle:    'bg-slate-800 border-slate-800',
    line:      'bg-slate-800',
...
const PRIORITY_CFG: Record<'TWK' | 'TIU' | 'TKP', {
  ...
  TWK: {
    topBar: 'bg-gradient-to-r from-sky-500 to-sky-600',
    bg: 'bg-sky-50/70', border: 'border-sky-200', btnBg: 'bg-sky-600',
    chipBg: 'bg-sky-600 text-white', color: 'text-sky-700',
...
const CAT_BAR: Record<string, { bar: string; badge: string; text: string }> = {
  TWK: { bar: 'bg-sky-500',    badge: 'bg-sky-100 text-sky-700',       text: 'text-sky-600'    },
    ...
    <div className="bg-white border-b border-slate-100">
      ...
      >
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-lg bg-slate-800/8 flex items-center justify-center flex-shrink-0">
            {icon}
          </div>
          <span className="text-sm font-extrabold text-slate-800" style={{ fontFamily: 'var(--font-jakarta)' }}>
            {title}
          </span>
          {badge}
        </div>
...
export function MobileRoadmap({ stats, savedPreference, studyHistory = [] }: MobileRoadmapProps) {
  ...
  const getDayStyle = (day: number) => {
    if (isExamDay(day)) return { wrapper: 'bg-slate-800 rounded-full', text: 'text-white font-bold' };
    if (isToday(day))   return { wrapper: 'bg-slate-800/10 rounded-full ring-2 ring-slate-800/30', text: 'text-slate-800 font-bold' };
    if (isPast(day)) {
    ...
    <main className="bg-slate-50 min-h-screen">
      ...
      <div className="px-4 py-3 bg-white border-b border-slate-100">
        {isAllDone ? (
          ...
          /* Default */
          <div className="rounded-2xl border border-slate-800/15 bg-slate-800/[0.04] px-4 py-3 flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-slate-800 flex items-center justify-center flex-shrink-0">
              <Lightbulb className="w-5 h-5 text-[#F5A623]" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[9px] font-extrabold text-slate-800/50 uppercase tracking-widest">Langkah Selanjutnya</p>
              <p className="text-xs font-bold text-slate-800 leading-snug mt-0.5 line-clamp-2">{rekomendasi.message}</p>
            </div>
            <Link href={rekomendasi.href}>
      ...
      <Section
        ...
        icon={<Target className="w-3.5 h-3.5 text-slate-800" />}
        badge={
          <span className="text-[10px] font-bold bg-slate-800/8 text-slate-800 px-2 py-0.5 rounded-full">
                        ...
                        <Link href={phase.ctaHref}>
                          <button className={cn(
                            ...
                            phase.status === 'active'
                              ...
                              : 'bg-white border border-slate-800/20 text-slate-800',
                          )}>
      ...
      <Section
        ...
        icon={<BarChart3 className="w-3.5 h-3.5 text-slate-800" />}
        badge={
          achievedCount > 0
        ...
        <div className="px-4 space-y-4">
          ...
          <div className="bg-slate-50 rounded-2xl border border-slate-100 overflow-hidden">
            <div className="flex items-center gap-2 px-3.5 py-2.5 border-b border-slate-100">
              ...
              <span className="text-xs font-extrabold text-slate-800">Skor per Kategori</span>
          ...
          <div className="bg-slate-50 rounded-2xl border border-slate-100 overflow-hidden">
            <div className="flex items-center justify-between px-3.5 py-2.5 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-extrabold text-slate-800">Pencapaian</span>
              </div>
            ...
            <div className="p-2.5 grid grid-cols-2 gap-2">
              {milestones.map(m => (
                ...
                >
                  ...
                  <p className={cn('text-xs font-extrabold leading-snug', m.achieved ? 'text-slate-800' : 'text-slate-400')}>
                    {m.label}
                  </p>
      ...
      <Section
        ...
        icon={<CalendarDays className="w-3.5 h-3.5 text-slate-800" />}
        badge={
        ...
        <div className="px-4 space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-50">
              ...
              <p className="text-sm font-extrabold text-slate-800 capitalize" style={{ fontFamily: 'var(--font-jakarta)' }}>
                {monthLabel}
              </p>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
            <div className="px-3.5 py-2.5 border-b border-slate-50">
              <p className="text-xs font-extrabold text-slate-800">Frekuensi Latihan</p>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isActive ? 'bg-slate-800/[0.05] border-slate-800/25' : 'bg-white border-slate-100',
                    )}
                  >
                    <div className="flex items-center gap-2">
                      {opt.value === 'custom' && <Pencil className="w-3 h-3 text-slate-400" />}
                      <div>
                        <span className={cn('text-xs font-bold block', isActive ? 'text-slate-800' : 'text-slate-600')}>
                          {opt.label}
                        </span>
                        {isActive && (
                    ...
                    <div className={cn('w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0', isActive ? 'border-slate-800 bg-slate-800' : 'border-slate-300')}>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
            <div className="px-3.5 py-2.5 border-b border-slate-50">
              <p className="text-xs font-extrabold text-slate-800">Pengingat Otomatis</p>
            </div>
